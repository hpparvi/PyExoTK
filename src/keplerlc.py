...
class KeplerLC(object):
    def __init__(self, time, flux, zero_epoch, period, d_transit, d_baseline, quarter=None, **kwargs):
        ...
        phase     = fold(time, period, zero_epoch, 0.5)
        ...
        self.tidarr, nt = label(msk_phase)                             ## transit indices
        ...
        for tid,ptp in enumerate(map(np.ptp, self.normalized_flux_per_transit)):
            if ptp > self.max_ptp:
                msk_inc[self.tidarr==tid] = 0
        self._compress_data(msk_inc)
        self._compute_indices()
    ...
    def _compute_indices(self): 
        ...
        self.orbit_n = np.round((np.array(map(np.mean, self.time_per_transit)) - self.t0) / self.p).astype(np.int)
